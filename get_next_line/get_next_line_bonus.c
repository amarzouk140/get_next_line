...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/05 18:03:35 by amarzouk          #+#    #+#             */
/*   Updated: 2024/01/25 23:53:27 by ayman_marzo      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || BUFFER_SIZE > INT_MAX)
		return (NULL);
	buffer[fd] = read_string(fd, buffer[fd]);
	if (!buffer[fd])
		return (NULL);
	line = copy_line(buffer[fd]);
	buffer[fd] = skip_and_free(buffer[fd]);
	return (line);
}
